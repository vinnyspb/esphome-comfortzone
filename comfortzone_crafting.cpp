...
//		  1 CRC byte
void czcraft::craft_w_cmd(comfortzone_heatpump *czhp, W_CMD *q, uint8_t *reg_num, uint16_t value)
{
	...
	q->crc = czhp->CRC8.maxim((uint8_t*)q, sizeof(W_CMD) - 1);
}

// craft a R_CMD paquet
// input: pointer to output buffer
//		  9 byte array contaning register number
//		  1 CRC byte
void czcraft::craft_r_cmd(comfortzone_heatpump *czhp, R_CMD *q, uint8_t *reg_num)
{
	uint8_t comp1_dest[4];
	...
	memcpy(q->cz_head.source, czhp->controller_addr, 4);
	q->cz_head.packet_size = sizeof(R_CMD);		// == 0x18
	q->cz_head.cmd = 'R';
	memcpy(q->cz_head.reg_num, reg_num, 9);

	q->wanted_reply_size = sizeof(R_REPLY_STATUS_06);

	q->crc = czhp->CRC8.maxim((uint8_t*)q, sizeof(R_CMD) - 1);
}
...
//		  8bit value
//		  1 CRC byte
void czcraft::craft_w_small_cmd(comfortzone_heatpump *czhp, W_SMALL_CMD *q, uint8_t *reg_num, uint8_t value)
{
	uint8_t comp1_dest[4];
	...
	q->crc = czhp->CRC8.maxim((uint8_t*)q, sizeof(W_SMALL_CMD) - 1);
}
...
//		9 byte array contaning register number
//		expected 8bit value
void czcraft::craft_w_reply(comfortzone_heatpump *czhp, W_REPLY *q, uint8_t *reg_num, uint8_t value)
{
	uint8_t comp1_dest[4];
	...
	q->crc = czhp->CRC8.maxim((uint8_t*)q, sizeof(W_REPLY) - 1);
}
