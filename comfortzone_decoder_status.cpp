...
static void dump_unknown(const char *prefix, uint8_t *start, int length)
{
	ESP_LOGD(TAG, prefix);
	ESP_LOG_BUFFER_HEX(TAG, start, length);
}

static void dump_float(const char* prefix, const char* suffix, uint8_t value[2])
{
	int reg_v = get_uint16(value);

	float reg_v_f = reg_v;
	reg_v_f /= 10.0;

	ESP_LOGD(TAG, "%s %f %s", prefix, reg_v_f, suffix);
}

static void dump_uint16(const char* prefix, const char* suffix, uint8_t value[2])
{
	int reg_v = get_uint16(value);
	ESP_LOGD(TAG, "%s %u %s", prefix, reg_v, suffix);
}

static void dump_uint8(const char* prefix, const char* suffix, uint8_t value)
{
	ESP_LOGD(TAG, "%s 0x%X %s", prefix, value, suffix);
}
...
// #ifdef DEBUG
	dump_float("Evaporator pressure:", "", q->evaporator_pressure);
	dump_float("Pressure ratio:", "", q->pressure_ratio);
	dump_unknown("unknown_s06_0a", q->unknown0a, sizeof(q->unknown0a));
	dump_float("Heatpump - Compressor max frequency (1) (erroneous):", "Hz", q->heatpump_compressor_max_frequency1);
	dump_float("Hot water - Compressor active max frequency:", "Hz", q->hot_water_active_max_frequency);
	dump_float("Heatpump - Compressor active max frequency (during defrost, set to 0Hz else real compressor max frequency) (1):", "Hz", q->heatpump_active_max_frequency1);
	dump_float("Heatpump - Compressor active max frequency (during defrost, set to 0Hz else real compressor max frequency) (2):", "Hz", q->heatpump_active_max_frequency2);
	dump_float("Heatpump - Compressor active max frequency (during defrost, set to 0Hz else real compressor max frequency) (3):", "Hz", q->heatpump_active_max_frequency3);
	dump_unknown("unknown_s06_0c", q->unknown0c, sizeof(q->unknown0c));
	dump_float("Heatpump - current compressor frequency:", "Hz", q->heatpump_current_compressor_frequency);
	dump_float("Chauffage - Compressor max frequency (3):", "Hz", q->chauffage_compressor_max_frequency3);
	dump_unknown("unknown_s06_0d", q->unknown0d, sizeof(q->unknown0d));
	dump_float("Heating - Compressor min frequency:", "Hz", q->heating_compressor_min_frequency);
	dump_float("Heating - Compressor max frequency:", "Hz", q->heating_compressor_max_frequency);
	dump_unknown("unknown_s06_0", q->unknown0, sizeof(q->unknown0));
	dump_uint16("Heatpump - current compressor power:", "W", q->heatpump_current_compressor_power);
	dump_uint16("Heatpump - current add power:", "W", q->heatpump_current_add_power);
	dump_uint16("Heatpump - current total power 1:", "W", q->heatpump_current_total_power1);
	dump_uint16("Heatpump - current total power 2:", "W", q->heatpump_current_total_power2);
	dump_uint16("Heatpump - Compressor input power:", "W", q->heatpump_compressor_input_power);
	dump_unknown("unknown_s06_1a", q->unknown1a, sizeof(q->unknown1a));
	dump_uint16("Heatpump - remaining min runtime (?):", "seconds", q->unknown_count_down);
	dump_unknown("unknown_s06_1b", q->unknown1b, sizeof(q->unknown1b));
	dump_uint16("Heatpump - remaining time to next defrost (=remaining max runtime):", "seconds", q->heatpump_defrost_delay);
	dump_unknown("unknown_s06_2", q->unknown2, sizeof(q->unknown2));
	dump_float("Expansion valve - Calculated setting:", "K", q->expansion_valve_calculated_setting);
	dump_float("Vanne expansion - xxx?:", "K", q->vanne_expansion_xxx);
	dump_float("Expansion valve - Temperature difference 1:", "K", q->expansion_valve_temperature_difference1);
	dump_float("Expansion valve - Temperature difference 2:", "K", q->expansion_valve_temperature_difference2);
	dump_unknown("unknown_s06_2a", q->unknown2a, sizeof(q->unknown2a));
	dump_float("Expansion valve - Valve position 1:", "%", q->expansion_valve_valve_position1);
	dump_float("Expansion valve - Valve position 2:", "%", q->expansion_valve_valve_position2);
	dump_unknown("unknown_s06_2a", q->unknown2a, sizeof(q->unknown2a));
	dump_uint8("crc:", "", q->crc);
// #endif
}
